...
get_channel_id() {
  ...
  resp="$(curl -sS "https://slack.com/api/conversations.list?exclude_archived=true&limit=1000" \
...
history="$(curl -sS "https://slack.com/api/conversations.history?channel=${CHANNEL_ID}&limit=${LIMIT}" \
  -H "Authorization: Bearer $SLACK_BOT_TOKEN")"

# Write triad_inbox.md + triad_handoff.json
python3 - <<'PY' "$TRIAD_SLACK_CHANNEL" <<< "$history"
...
open("triad_inbox.md", "w", encoding="utf-8").write("\n".join(lines))
...
if handoff["next"]:
  ...
  open("triad_handoff.json", "w", encoding="utf-8").write(json.dumps(out, indent=2))
else:
  # Write an empty file so scripts don't fail
  open("triad_handoff.json", "w", encoding="utf-8").write(json.dumps({"channel": channel_name, "next": None}, indent=2))
